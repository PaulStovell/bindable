...
namespace Bindable.Linq.Dependencies.ExpressionAnalysis
{
    ...
    public sealed class ExpressionFlattener
    {
        ...
        {
            TraverseExpressions(listInitExpression.Initializers.Select(i => i.Arguments.Cast<Expression>()).UnionAll());
        ...
        private void TraverseMemberInitExpression(MemberInitExpression memberInitExpression)
        {
            ...
            TraverseExpressions(memberInitExpression.Bindings.Where(b => b.BindingType == MemberBindingType.ListBinding).Select(b => ((MemberListBinding) b).Initializers.Select(i => i.Arguments.Cast<Expression>()).UnionAll()));
            TraverseExpressions(memberInitExpression.Bindings.Where(b => b.BindingType == MemberBindingType.MemberBinding).Select(b => ((MemberMemberBinding) b).Bindings.Cast<Expression>()).UnionAll());
